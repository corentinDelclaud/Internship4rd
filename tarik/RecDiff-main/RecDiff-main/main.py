...
from retrieval import retrieval_via_pcst  # Add this import
...
class Coach:
    def __init__(self, handler):
        ...
        # --- Denoise graphs using PCST retrieval ---
        q_emb = self.handler.ui_graph.x if hasattr(self.handler.ui_graph, 'x') else torch.zeros((self.handler.ui_graph.num_nodes(), 16))
        textual_nodes = getattr(self.handler, 'textual_nodes', None)
        textual_edges = getattr(self.handler, 'textual_edges', None)
        if textual_nodes is not None and textual_edges is not None:
            self.uiGraph, _ = retrieval_via_pcst(self.handler.ui_graph, q_emb, textual_nodes, textual_edges, topk_e=15, cost_e=0.5)
            self.uuGraph, _ = retrieval_via_pcst(self.handler.uu_graph, q_emb, textual_nodes, textual_edges, topk_e=15, cost_e=0.5)
            self.uiGraph = self.uiGraph.to(self.device)
            self.uuGraph = self.uuGraph.to(self.device)
        else:
