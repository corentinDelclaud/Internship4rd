...
def get_train_data(aug_model, augments, tokenizer, args):
    ...
    prompt_ids = []
    for aug in augments:
        if aug_model == "meta-llama/Llama-3.2-1B-Instruct":
            psg = aug["passage"]
            rew = aug[f"llama3.2-1b-instruct_rewrite"]
            qas = aug[f"llama3.2-1b-instruct_qa"]
        elif aug_model == "meta-llama/Meta-Llama-3-8B-Instruct":
            psg = aug["passage"]
            rew = aug[f"llama3-8b-instruct_rewrite"]
            qas = aug[f"llama3-8b-instruct_qa"]
        elif aug_model == "Qwen/Qwen2.5-1.5B-Instruct":
            psg = aug["passage"]    
            rew = aug[f"qwen2.5-1.5b-instruct_rewrite"]
            qas = aug[f"qwen2.5-1.5b-instruct_qa"]
        else:
            psg = aug["passage"]
